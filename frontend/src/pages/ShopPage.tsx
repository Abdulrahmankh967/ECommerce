...
export const ShopPage: React.FC = () => {
  ...
  // 1️⃣ حالة جديدة لتخزين القيمة المؤخرة (Debounced Value)
  ...
  // 2️⃣ تطبيق الـ Debounce لمنع إرسال طلبات سريعة عند كل تغيير حرف
  useEffect(() => {
    const timer = setTimeout(() => {
      const trimmed = searchQuery.trim();
      setDebouncedSearchQuery(trimmed);

      // تحديث الـ URL مع الـ Debounce
      if (trimmed) {
  ...
  // 3️⃣ استخدام `debouncedSearchQuery` بدلاً من `searchQuery` المباشر
  const { data: allProducts = [], isLoading: isProductsLoading } = useQuery({
    queryKey: ['products', debouncedSearchQuery],
    queryFn: () =>
        debouncedSearchQuery
            ? productsApi.searchProducts(debouncedSearchQuery)
  ...
  // Apply Client-Side filters (Category, Price, Stock) on the Elasticsearch results
